...
def download_CNEMS(now):  
    ...
    for atoll,sla_lon,sla_lat,sla_off in zip(atoll_list,sla_lon_list,sla_lat_list,sla_offset_list):
        ...
        os.system('python -m motuclient --motu http://nrt.cmems-du.eu/motu-web/Motu\
                              ...
                              --user aespejo --pwd Copernicus580000!' %{"lon_min":(sla_lon-0.001),"lon_max":(sla_lon+0.001),"lat_min":(sla_lat-0.001),"lat_max":(sla_lat+0.001),"ini":now.strftime('%Y-%m-%d %H:%M:%S') ,"end":then.strftime('%Y-%m-%d %H:%M:%S'),"folder":folder_tmp,"island":atoll})
