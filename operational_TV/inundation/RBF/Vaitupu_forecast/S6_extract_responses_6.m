...
%%
fff = 0;
for i = 6:6:length(transect_labels)
    dirName1 = [base_path '/t_' num2str(t{i}.label) '_base'];
    load([dirName1 '/centroid.mat']);
    dist = t{i}.dist';
    dist = -dist';
    ...
    for j = 1:length(subset)
        ...
        try
            ...
            run_path = '/scale_wlg_nobackup/filesets/nobackup/spc03223/TCAP/Forecast/Vaitupu_forecast/Failed_runs_6/';
...
% 
% fid  = fopen('Failed_runs_6/run_all_models_1.sh','w');
% for j = 1:700
%     fprintf(fid,'%s',['sbatch failed_job_' num2str(j) '.sl']);
%     fprintf(fid,'\n');
% end
% fclose(fid);
% fid  = fopen('Failed_runs_6/run_all_models_2.sh','w');
% for j = 701:1400
%     fprintf(fid,'%s',['sbatch failed_job_' num2str(j) '.sl']);
%     fprintf(fid,'\n');
% end
% fclose(fid);
% fid  = fopen('Failed_runs_6/run_all_models_3.sh','w');
% for j = 1401:fff
%     fprintf(fid,'%s',['sbatch failed_job_' num2str(j) '.sl']);
%     fprintf(fid,'\n');
% end
...
% cnt = 0;
% for i = 1:20
% fid  = fopen(['Failed_runs_6/run_all_models_' num2str(i) '.sh'],'w');
% for j = 1:1000
%     cnt = cnt+1;
...
run_path = '/nesi/nobackup/spc03223/TCAP/Forecast/Vaitupu_forecast/Failed_runs_6';
fid  = fopen(['Failed_runs_6/Run_failed_jobs' '.sl'],'w');
fprintf(fid,'%s',['#!/bin/bash -e']);
fprintf(fid,'\n');
fprintf(fid,'%s',['#SBATCH --job-name=XBeach_Vaitupu # job name (shows up in the queue)']);
fprintf(fid,'\n');
