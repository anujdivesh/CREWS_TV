...
%%
run_path = '/nesi/nobackup/spc03223/TCAP/Forecast/Funafuti_forecast/Failed_runs';
fid  = fopen(['Failed_runs/Run_failed_jobs' '.sl'],'w');
fprintf(fid,'%s',['#!/bin/bash -e']);
fprintf(fid,'\n');
fprintf(fid,'%s',['#SBATCH --job-name=XBeach_Funafuti # job name (shows up in the queue)']);
fprintf(fid,'\n');
