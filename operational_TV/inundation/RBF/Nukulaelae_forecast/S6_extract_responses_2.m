...
path_transect = 'G:\XBeach\Tuvalu\1D_sims\Nukulaelae_forecast\Nukulaelae_forecast\Nukulaelae\Nukulaelae_Oceanside_Profilesv3.csv';
path_eva = 'F:\XBeach\Tuvalu\1D_sims\Nukulaelae\';
path_tcs = [path_eva 'TC_Nukulaelae_synth.mat'];
base_path = 'G:\XBeach\Tuvalu\1D_sims\Nukulaelae_forecast\Nukulaelae_forecast';
...
%%
fff = 0;
for i = 272+6:6:length(transect_labels)
    dirName1 = [base_path '/t_' num2str(t{i}.label) '_base'];
    load([dirName1 '/centroid.mat']);
    dist = t{i}.dist';
    dist = -dist';
    ...
    for j = 1:length(subset)
        ...
        try
            ...
            run_path = '/scale_wlg_nobackup/filesets/nobackup/spc03223/TCAP/Forecast/Nukulaelae_forecast/Failed_runs_2/';
            fid  = fopen(['Failed_runs_2/failed_job_' num2str(fff) '.sl'],'w');
            fprintf(fid,'%s',['#!/bin/bash -e']);
            fprintf(fid,'\n');
            fprintf(fid,'%s',['#SBATCH --job-name=XBeach_Nukulaelae # job name (shows up in the queue)']);
            fprintf(fid,'\n');
...
run_path = '/nesi/nobackup/spc03223/TCAP/Forecast/Nukulaelae_forecast/Failed_runs_2';
