...
def inundation_forecast(now):
    ...
    fl_name_tides = '../tmp/tide_hourly_Vaitupu.nc'
    fl_name_tides_min = '../tmp/tide_minute_Vaitupu.nc'
    fl_name_sla = '../tmp/sla_hourly_Vaitupu.nc'
    ...
    transects_csv = '../inundation/RBF/Vaitupu_forecast/Vaitupu/Vaitupu_Oceanside_Profilesv3.csv'
    ...
    for tt in range(len(transect_labels)):
        fig_outfile_name = '../inundation/Figures/Vaitupu_t_' + str(transect_labels[tt]) + '_forecast.png'
        risk_outfile_name = '../inundation/Flood_risk/Vaitupu_t_' + str(transect_labels[tt]) + '_risk_level.csv'
        
        coordinate_fl = '../inundation/RBF/Vaitupu_forecast/t_' + str(transect_labels[tt]) + '_coordinates.csv'
        training_data_path = '../inundation/RBF/Vaitupu_forecast/t_' + str(transect_labels[tt]) + '_results.csv'
        rbf_coeff_path = '../inundation/RBF/Vaitupu_forecast/t_' + str(transect_labels[tt]) + '_rbf_coefficients_max_TWL_and_inun_ext.pkl'
        thresholds_path = '../inundation/RBF/Vaitupu_forecast/t_' + str(transect_labels[tt]) + '_thresholds.csv'
